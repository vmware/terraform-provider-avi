...
* `diameter_app_persistence_profile` - Specifies the diameter persistence profile parameters. Field introduced in 31.1.1. Allowed with any value in enterprise, essentials, basic, enterprise with cloud services edition.
* `hdr_persistence_profile` - Specifies the custom http header persistence profile parameters. Allowed with any value in enterprise, essentials, basic, enterprise with cloud services edition.
...
* `persistence_update_interval` - Interval in minutes at which refreshed persistence entries are synced to peer ses. If not set, it willsync at an interval of timeout/2. Allowed values are 1-30. Field introduced in 30.2.4. Unit is min. Allowed with any value in enterprise, essentials, basic, enterprise with cloud services edition.
* `server_hm_down_recovery` - Specifies behavior when a persistent server has been marked down by a health monitor. Enum options - HM_DOWN_PICK_NEW_SERVER, HM_DOWN_ABORT_CONNECTION, HM_DOWN_CONTINUE_PERSISTENT_SERVER. Allowed with any value in enterprise, essentials, basic, enterprise with cloud services edition.
