...
* `graceful_disable_timeout` - Used to gracefully disable a server. Virtual service waits for the specified time before terminating the existing connections  to the servers that are disabled. Not used anymore .graceful_disable_timeout_sec is introduced. Allowed values are 1-7200. Special values are 0 - immediate, -1 - infinite. Unit is min. Allowed with any value in enterprise, essentials, basic, enterprise with cloud services edition.
