...
* `local_password_managed_externally` - (Optional) Indicates that this local user's password is managed externally (e.g. By vcf/sddc manager) rather than by avi. Field introduced in 32.1.3. Allowed with any value in enterprise, essentials, basic, enterprise with cloud services edition. 
