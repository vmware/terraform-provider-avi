...
* `ignore_vrf_in_networksubnetlist` - (Optional) Ignore the vrf_context filter for /networksubnetlist api. Field introduced in 30.2.1. Allowed in enterprise edition with any value, enterprise with cloud services edition.
...
* `seupgrade_copy_buffer_size` - (Optional) This parameter defines the buffer size during se image downloads in a segroup. It is used to pace the se downloads so that controller network/cpu bandwidth is a bounded operation. Field introduced in 22.1.4, 30.1.1. Allowed in enterprise edition with any value, enterprise with cloud services edition.
