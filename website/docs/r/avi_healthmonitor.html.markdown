...
* `monitor_ip` - (Optional) Destination ip address to be monitored instead of the pool member ip. Field introduced in 31.2.1. Allowed with any value in enterprise, enterprise with cloud services edition. Changing this value forces the resource to be recreated.
