...
* `persistence_update_interval` - (Optional) Interval in minutes at which refreshed persistence entries are synced to peer ses. If not set, it willsync at an interval of timeout/2. Allowed values are 1-30. Field introduced in 31.1.2. Unit is min. Allowed with any value in enterprise, enterprise with cloud services edition.
