...
type UpgradeStatusInfo struct {
	...
	// Backward compatible abort function name. Field introduced in 18.2.10.
	AfterRebootRollbackFnc *string `json:"after_reboot_rollback_fnc,omitempty"`

	// Backward compatible task dict name. Field introduced in 18.2.10.
	AfterRebootTaskName *string `json:"after_reboot_task_name,omitempty"`

	// Flag for clean installation. Field introduced in 18.2.10.
	...
	// Image path of current base image. Field introduced in 18.2.10.
	...
	// Flag for patch op with reboot. Field introduced in 18.2.10.
	...
	// Image path of previous base image. Field introduced in 18.2.10.
	PrevImagePath *string `json:"prev_image_path,omitempty"`

	// Image path of previous patch image. Field introduced in 18.2.10.
	...
	// Image path of se patch image.(required in case of reimage and upgrade + patch). Field introduced in 18.2.10.
	SePatchImagePath *string `json:"se_patch_image_path,omitempty"`

	// Image uuid for identifying the current se patch required in case of system upgrade(re-image) with se patch. . It is a reference to an object of type Image. Field introduced in 18.2.10.
	...
	// se_patch may be different from the controller_patch. It has to be saved in the journal for subsequent consumption. The SeGroup params will be saved in the controller entry as seg_params. . Field introduced in 18.2.10.
