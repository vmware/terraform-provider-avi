...
func TestAVISePropertiesBasic(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy: testAccCheckAVISePropertiesDestroy,
		Steps: []resource.TestStep{
			{
				Config: testAccAVISePropertiesConfig,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckAVISePropertiesExists("avi_seproperties.testSeProperties"),
				),
			},
			{
...
func testAccCheckAVISePropertiesExists(resourcename string) resource.TestCheckFunc {
...
func testAccCheckAVISePropertiesDestroy(s *terraform.State) error {
	conn := testAccProvider.Meta().(*clients.AviClient).AviSession
	var obj interface{}
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "avi_seproperties" {
			continue
		}
		url := strings.SplitN(rs.Primary.ID, "/api", 2)[1]
		uuid := strings.Split(url, "#")[0]
		...
		if len(obj.(map[string]interface{})) > 0 {
			return fmt.Errorf("AVI SeProperties still exists")
		}
	}
	return nil
}
