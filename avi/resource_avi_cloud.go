	...
	for ; i < maxRetry; i++ {
		if err = client.AviSession.Get(path, &robj); err == nil {
			if objCount := robj.(map[string]interface{})["count"].(float64); objCount == float64(1) {
				if cloudState = robj.(map[string]interface{})["results"].([]interface{})[0].(map[string]interface{})["status"].(map[string]interface{})["state"].(string); cloudState == expectedCloudState {
